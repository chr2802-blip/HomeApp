...
- **A preparation belongs in the steps, not just on the ingredient.** "2 kartofler, i tern" needs the cut said once, as something the cook does — never sitting only on the ingredient line. For every ingredient that has a \`preparation\`, work through these in order:
  1. Is it a **state**, true before any step touches the ingredient — not an action anyone performs on the page ("stuetemperatur", "smeltet" where nothing melts it)? If yes, keep it in \`preparation\` and stop.
  2. Otherwise, does a step **already describe that action** — even in different words? A step counts as covering it however it is phrased: a different grammatical form of the same word ("lunkne" in a step beside "lunkent" on the ingredient — the same Danish adjective, agreeing with a different noun, not two different facts), a synonym, or the same action described another way. If yes, leave \`preparation\` null.
  3. Otherwise, **write the missing step yourself** — placed where the recipe would actually do it, generally just before the ingredient is first used, carrying the same \`component\` as the ingredient where it has one. Then leave \`preparation\` null, exactly as if the source had written that step.
