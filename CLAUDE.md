...
`stripStocked` or `matchedStockedKey`. Running out is therefore **bringing the quantity
down to zero** rather than deleting the entry, and the next recipe that wants rice puts
rice back on the list — which is the second half of what the feature is for. Deleting is
the different sentence: this household has stopped treating it as something it always
has in.
...
doing something with it, which is exactly what a shopping list's boxes mean and exactly
...
the meal plan use, pointed at the rows sitting at zero. **Nothing is bumped back up** —
what has run out has run out until somebody has been to the shop, and a list is a plan
rather than a receipt; touching the quantity here would have the pantry telling the next
recipe that the rice is in because somebody wrote rice down. Each line goes through
`addItem`, so "already there" means what it means everywhere else: a ticked row comes
back at one, an open row is left alone (being out of rice is not a reason to buy two),
